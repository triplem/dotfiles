...
stolen from: 
https://github.com/sruffell/timewarrior-timesheet-report/commit/62db211411caee78cfa0bcba870aa2fd6ea56575#diff-06e25761e519f43c7967bbb3795aa6015b6a0d824111e0b865d206259d969049
"""
...
TENTHS_OF_HOURS = Decimal('0.01')
SECONDS_IN_HOUR = Decimal('3600.00')
...
class Interval(object):
    ...
    def __init__(self, interval_string, config):
        ...
        projects = [x for x in json_object["tags"] ]
    ...
    def month(self):
        """Return the month number for this interval."""
        return self._month - 1

    def year(self):
        """Return the year for this interval."""
        return self._year
...
class Report(object):
    ...
    def _intervals_to_data(intervals):
        ...
        for interval in intervals:
            if interval.tag() not in projects:
                projects[interval.tag()] = [0]*12
            projects[interval.tag()][interval.month()] += interval.total_seconds()
        ...
        for key in project_keys:
            ...
            for month, total_seconds in enumerate(projects[key]):
                value = (Decimal(total_seconds) / SECONDS_IN_HOUR)
                row[month] = value.quantize(TENTHS_OF_HOURS,
                                            rounding=decimal.ROUND_HALF_UP)
            rows.append((key, row))

        # Now sum up the totals
        monthly_totals = [Decimal()]*12
        for _, data in rows:
            project_total = Decimal()
            for month, value in enumerate(data):
                project_total += value
                monthly_totals[month] += value
            data.append(project_total)
        monthly_totals.append(sum(monthly_totals, Decimal()))
        rows.append(('totals', monthly_totals))
        return rows
    ...
    def _print_header(self, put_fn):
        put_fn("{:{}s} |".format(" " * self._tag_width, self._column_width))
        for month in ("Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"):
            put_fn(" {:>{}s} |".format(month, self._column_width))
        put_fn("\n")
    ...
    def _write_row_data(self, put_fn, row):
        for month in row:
            if month > 0:
                put_fn("{:{}f} | ".format(month, self._column_width))
            else:
